...
An Azure DevOps pipeline is used to automatically build the image and deploy to Kubernetes. A pipeline environment is used to monitor and trace deployment history to the cluster includng the linked commit history. The initial deployment uses one 'test' pipeline environment. Additional environments should be added after discussing release management strategy with the development team.
...
The current Azure pipeline only has one deploy stage to a test environment and needs to be extended to promote the image to a production environment once tested and approved. This needs to be added after discussing dev / test strategy with the team.
